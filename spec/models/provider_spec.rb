...
RSpec.describe Provider, type: :model do
  ...
  describe "methods" do
    ...
    it "returns a true if accepts medicare" do
      expect(@takes_both.accepts_medicare).to eq(true)
      expect(@takes_care.accepts_medicare).to eq(true)
    end

    it "returns a false if not accepts medicare" do
      expect(@takes_neither.accepts_medicare).to eq(false)
      expect(@takes_caid.accepts_medicare).to eq(false)
    end

    it "returns a true if accepts medicaid" do
      expect(@takes_both.accepts_medicaid).to eq(true)
      expect(@takes_caid.accepts_medicaid).to eq(true)
    end

    it "returns a false if not accepts medicaid" do
      expect(@takes_neither.accepts_medicaid).to eq(false)
      expect(@takes_care.accepts_medicaid).to eq(false)
    end
  end
end
